...
create PROCEDURE [dbo].[PR_CalcularForecastCD]
AS
BEGIN
	SET NOCOUNT ON
	...
	BEGIN
		...
		FROM (
			...
			INNER JOIN Loja l
				ON l.cdLoja = F.cdLoja
					AND l.cdSistema = F.cdFormato
			JOIN CD ON CD.cdCD = F.cdCd
			INNER JOIN VW_REAB_SugestaoPedidoItensRelacionados SP
				ON SP.idCd = CD.cdCd
	...
	FROM (
		...
		FROM WLMSLP_STAGE..SugestaoPedidoCD S
		INNER JOIN VW_REAB_SugestaoPedidoItensRelacionados IR
