/****** Object:  StoredProcedure [dbo].[PR_ReportItensTransformados]    Script Date: 05/10/2016 14:53:10 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[PR_ReportItensTransformados]
	@cdSistema				tinyint,
...
FROM (   
	...
	SELECT  
		...
		ID.cdOldNumber as ItemTransformado,
		NULL as ItemInsumo,  
		ID.dsItem,
		ID.dsHostItem,
		ID.cdUPC,
		...
		ID.cdPLU,  
		ID.dsTamanhoItem,
		RIP.pcRendimentoReceita AS Rendimento,
		psItem = 0,
		RIP.psUnitario,
		qtItemUn = 0,
		0 AS Tipo,
		...
		ACD.dsAreaCD,
		ID.vlModulo as PesoProdutoAcabado,
		ID.vlPesoLiquido as PesoUnitario,
  ...
  UNION ALL  
	...
	SELECT  
		...
		CONVERT(VARCHAR, sc.cdSubcategoria) + ' - ' + SC.dsSubcategoria AS dsSubcategoria,
		CONVERT(VARCHAR, FL.cdFineLine) + ' - ' + FL.dsFineLine AS dsFineline,
		IDP.cdOldNumber as ItemTransformado,
		IDS.cdOldNumber as ItemInsumo,  
		IDS.dsItem,
		IDS.dsHostItem,
		IDS.cdUPC,
		F.cdFornecedor,
		...
		ACD.dsAreaCD,
		IDS.vlModulo as PesoProdutoAcabado,
		IDS.vlPesoLiquido as PesoUnitario,
