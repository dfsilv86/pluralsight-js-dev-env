...
ALTER VIEW [dbo].[VW_REAB_SugestaoPedidoItensRelacionados]
AS
WITH Itens (IDRelacionamentoItemSecundario, IdItemDetalheS, dsItemS, cdItemS, IdItemDetalheP, dsItemP, cdItemP,tpVinculado, tpManipulado, tpReceituario, TotalParticipacaoReceita, TotalRendimentoReceita, PesoUnitatio, TotalParticipacaoReceitaOriginal, TotalRendimentoReceitaOriginal, idCd)
AS
(	--Pai / Derivado
	SELECT RS.IDRelacionamentoItemSecundario IDRelacionamentoItemSecundario,
	  ...
	  FROM RelacionamentoItemPrincipal RP WITH(NOLOCK) INNER JOIN RelacionamentoItemSecundario RS WITH(NOLOCK)
										  ...
										  INNER JOIN Departamento DS WITH(NOLOCK)
												 ...
												 AND DS.blAtivo = 1
										  INNER JOIN WLMSLP_STAGE..SugestaoPedidoItemEntrada IEP WITH(NOLOCK)
												  ON IEP.ID_ITEM_SAIDA = RP.IdItemDetalhe	 
	...
	SELECT RS.IDRelacionamentoItemSecundario IDRelacionamentoItemSecundario,
	  ...
	  FROM RelacionamentoItemPrincipal RP WITH(NOLOCK) INNER JOIN RelacionamentoItemSecundario RS WITH(NOLOCK)
										  ...
										  INNER JOIN Departamento DS WITH(NOLOCK)
												 ...
												 AND DS.blAtivo = 1
										  INNER JOIN WLMSLP_STAGE..SugestaoPedidoItemEntrada IEP WITH(NOLOCK)
												  ON IEP.ID_ITEM_SAIDA = RS.IdItemDetalhe	 
...
SELECT DISTINCT I.*, L.IdLoja, L.CdLoja
  FROM Itens I WITH(NOLOCK) INNER JOIN Trait T WITH(NOLOCK) ON T.IdItemDetalhe = I.IdItemDetalheS
								 AND T.blAtivo = 1
								 AND T.cdSistema = 1
			   INNER JOIN Loja L WITH(NOLOCK) ON L.IdLoja = T.IdLoja
								AND L.cdSistema = 1
								AND L.blAtivo = 1
