/*
DECLARE @dtSolicitacao VARCHAR(MAX), @idDepartamento INT, @idCD INT, @idItem INT, @idFornecedorParametro INT, @statusPedido INT, @idSugestaoPedidoCD INT, @ItemPesoVariavel INT;
...
SET @ItemPesoVariavel = 0; --2:Todos 1:Sim 0:Não
--*/

SELECT 
		   SPC.idSugestaoPedidoCD, 
		   SPC.idFornecedorParametro, 
		   SPC.idItemDetalhePedido, 
		   SPC.idItemDetalheSugestao, 
		   SPC.idCD, 
		   SPC.dtPedido, 
		   SPC.dtEnvioPedido, 
		   SPC.dtCancelamentoPedido, 
		   SPC.dtCancelamentoPedidoOriginal, 
		   SPC.dtInicioForecast, 
		   SPC.dtFimForecast, 
		   SPC.tpWeek, 
		   SPC.tpInterval, 
		   SPC.cdReviewDate, 
		   SPC.vlLeadTime, 
		   SPC.qtVendorPackage, 
		   SPC.vlEstoqueSeguranca, 
		   SPC.tempoMinimoCD, 
		   SPC.tpCaixaFornecedor, 
		   SPC.vlPesoLiquido, 
		   SPC.vlTipoReabastecimento, 
		   SPC.vlCusto, 
		   SPC.qtdPackCompra, 
		   SPC.qtdPackCompraOriginal, 
		   SPC.qtdOnHand, 
		   SPC.qtdOnOrder, 
		   SPC.qtdForecast, 
		   SPC.qtdPipeline, 
		   SPC.IdOrigemDadosCalculo, 
		   SPC.blFinalizado, 
		   SPC.tpStatusEnvio, 
		   SPC.dhEnvioSugestao,
		   SPC.vlReviewTime,
		   NULL AS SplitOn1,
		   ItemDetalhePedido.cdItem, 
		   ItemDetalhePedido.dsItem, -- Entrada
		   NULL AS SplitOn2,
		   FP.cdV9D, 
		   FP.IDFornecedor, 
		   F.nmFornecedor, 
		   ...
		   NULL AS SplitOn5,
		   ODC.dsOrigem,
		   ItemDetalheSugestao.cdItem AS cdItemSugestao, 
		   ItemDetalheSugestao.dsItem AS dsItemSugestao -- Saida
	...
	WHERE (
	SPC.idSugestaoPedidoCD = @idSugestaoPedidoCD) 
		OR (
			@idSugestaoPedidoCD IS NULL 
			AND (SPC.dtPedido = @dtSolicitacao
		AND SPC.idCD = @idCD
		...
		AND ((FP.IDFornecedor = (SELECT TOP 1 IDFornecedor FROM FornecedorParametro WHERE IDFornecedorParametro = @idFornecedorParametro)) OR @idFornecedorParametro IS NULL)
			...
			AND (@ItemPesoVariavel = 2 
				 OR (@ItemPesoVariavel = 1 AND (SELECT COUNT(idItemDetalhePedido) FROM SugestaoPedidoCD WHERE idItemDetalheSugestao = SPC.idItemDetalheSugestao AND dtPedido = @dtSolicitacao) > 1) 
				 OR (@ItemPesoVariavel = 0 AND (SELECT COUNT(idItemDetalhePedido) FROM SugestaoPedidoCD WHERE idItemDetalheSugestao = SPC.idItemDetalheSugestao AND dtPedido = @dtSolicitacao) = 1))
			)
	)
