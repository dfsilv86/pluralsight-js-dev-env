...
DECLARE @TmpEstoqueSelecionado TABLE (
	IDEstoque BIGINT,
	dtRecebimento DATETIME,
...
INSERT INTO @TmpEstoqueSelecionado
SELECT E.IDEstoque
     , NFI.dtRecebimento
  FROM @TmpItemDetalheSelecionado IDS
...
SELECT RowConstrainedResult.*
  FROM (
        SELECT ROW_NUMBER() OVER ( ORDER BY {2} ) AS RowNum, __INTERNAL.*
		  FROM (
				SELECT TOP 100 PERCENT (CONVERT(VARCHAR, L.cdLoja) + ' - ' + L.nmLoja) AS Loja
					 , L.cdLoja
					 , L.IDLoja
					 , E.IDEstoque
					 , IDE.cdSistema
					 ...
					 , IDE.cdItem
					 , E.dtRecebimento
					 , CONVERT(NUMERIC(10,2),E.vlCustoCompraAtual) CustoCompra
					 , CONVERT(NUMERIC(10,2),E.vlCustoGerencialAtual) CustoGerencial
					 , CONVERT(NUMERIC(10,2),E.vlCustoCadastroAtual) CustoCadastro
					 , CONVERT(NUMERIC(10,2),E.vlCustoContabilAtual) CustoContabil
					 , E.blCustoCadastro
					 , E.vlCustoMedio
					 , ES.dtRecebimento AS dtRecebimentoNota
				  FROM @TmpEstoqueSelecionado ES
					   INNER JOIN Estoque E WITH (NOLOCK)
							   ON E.IDEstoque = ES.IDEstoque
					   INNER JOIN @TmpItemDetalheSelecionado IDE
							   ON IDE.IDItemDetalhe = E.IDItemDetalhe
					   INNER JOIN @TmpLojasPermitidas L
							   ON L.IDLoja = E.IDLoja
			   ) __INTERNAL
