...
SET @cdSistema = 1;
--*/
WITH TipoItemTable AS (
	SELECT 0 AS blPesadoCaixa, 0 AS blPesadoRetaguarda, 'F' AS TipoItem
	UNION ALL
	SELECT 1 AS blPesadoCaixa, 0 AS blPesadoRetaguarda, 'V' AS TipoItem
	UNION ALL
	SELECT 0 AS blPesadoCaixa, 1 AS blPesadoRetaguarda, 'V' AS TipoItem
	UNION ALL
	SELECT 1 AS blPesadoCaixa, 1 AS blPesadoRetaguarda, 'V' AS TipoItem
), Itens AS 
(
...
FROM (
	SELECT ROW_NUMBER() OVER ( ORDER BY {2} ) AS RowNum, i.*
	FROM Itens i
	     INNER JOIN ItemDetalhe IDO ON IDO.IDItemDetalhe = i.IDItemDetalhe -- traz itemdetalhe para aqui para poder ordenar pelos campos
		 INNER JOIN TipoItemTable TI ON TI.blPesadoCaixa = IDO.blPesadoCaixa AND TI.blPesadoRetaguarda = IDO.blPesadoRetaguarda
	) AS Tb
	JOIN ItemDetalhe IDS WITH (NOLOCK)
			ON IDS.IDItemDetalhe = tb.IDItemDetalhe
	INNER JOIN Fornecedor F WITH (NOLOCK)
			ON F.IDFornecedor = IDS.IDFornecedor
	INNER JOIN FineLine FL WITH (NOLOCK)
			ON FL.IDFineLine = IDS.IDFineLine
