/*
DECLARE @cdItemSaida INT, @dsEstado VARCHAR, @IDRegiaoCompra INT, @IDBandeira INT, @blVinculado BIT, @idUsuario INT, @cdSistema INT, @TipoPermissao INT, @idFornecedorParametro INT;
SET @cdItemSaida = 9489020;
SET @dsEstado = NULL;
...
/*
	Pablo Brodt - 05/10/2016
	Para qualquer alteração nessa query é necessária uma verificação do impacto na query de exportação da tela.
	Query de exportação: RelacaoItemLojaCD.ExportacaoCadastroReabastecimento.sql
*/
...
SELECT RowConstrainedResult.*
  FROM ( 
        SELECT ROW_NUMBER() OVER ( ORDER BY {2} ) AS RowNum, __INTERNAL.*
		  FROM (
				SELECT RILC.IDRelacaoItemLojaCD
					, RILC.IDItem
					...
					, IDE.dsItem
					, IDE.IDFornecedor
					, dbo.fnObterTipoReabastecimento(RILC.IdItemEntrada, CD.IDCD, L.IDLoja) as vlTipoReabastecimento
					, NULL AS SplitOn3
					, FE.cdFornecedor
					, FE.nmFornecedor
